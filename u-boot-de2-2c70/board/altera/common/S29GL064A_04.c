...
//#define FIRST_ADDR	( CFG_FLASH_BASE + 0x0555 )
//#define SECOND_ADDR	( CFG_FLASH_BASE + 0x02AA )
...
#define FIRST_ADDR	( CFG_FLASH_BASE + 0x0AAA )
#define SECOND_ADDR	( CFG_FLASH_BASE + 0x0555 )
...
{
//	CFG_FLASH_WORD_SIZE *addr = (CFG_FLASH_WORD_SIZE *) info->start[0];
	CFG_FLASH_WORD_SIZE *addr2;
	int prot, sect;
	ulong start;
...
int write_buff (flash_info_t * info, uchar * src, ulong addr, ulong cnt)
{
	...
	CFG_FLASH_WORD_SIZE *dst = (CFG_FLASH_WORD_SIZE *) addr;
	CFG_FLASH_WORD_SIZE b;
	ulong start;
	while (cnt) {
		/* Check for sufficient erase */
		b = *(CFG_FLASH_WORD_SIZE *)src;
		if ((readw (dst) & b) != b) {
			printf ("%04x : %04x\n", readw (dst), b);
			return (2);
		}
